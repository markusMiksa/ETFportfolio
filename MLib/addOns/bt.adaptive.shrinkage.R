...
# average correlation shrinkage based on David's example
cov.average.david <- function(hist) {		 	
	n = ncol(hist)
...
#		periodicity = 'weeks', lookback.len = 252,
		...
		shrinkage.fns = list(
			...
			A50=shrink.average.david(0.5),

			S_SA_A=function(h,a) 1/3*(shrink.average.david(1)(h,a)
			+ sample.shrinkage(h,a) + sample.anchored.shrinkage(h,a)),			
						
			A_S=shrink.average.david(),
			D_S=shrink.diag(),					
			CC_S=shrink.const.cor(),
			SI_S=shrink.single.index(),
			SI2_S=shrink.two.parameter(),
			AVG_S=function(h,a) cov.shrink(h, 1/5*(shrink.average.david(1)(h,a)
			+ shrink.const.cor(1)(h,a) + shrink.diag(1)(h,a)
			+ shrink.single.index(1)(h,a) + shrink.two.parameter(1)(h,a)))$sigma,
			
			A=shrink.average.david(1),
			D=shrink.diag(1),					
			CC=shrink.const.cor(1),
			SI=shrink.single.index(1),
			SI2=shrink.two.parameter(1),
			AVG=function(h,a) 1/5*(shrink.average.david(1)(h,a)
			+ shrink.const.cor(1)(h,a) + shrink.diag(1)(h,a)
			+ shrink.single.index(1)(h,a) + shrink.two.parameter(1)(h,a))			
			),
