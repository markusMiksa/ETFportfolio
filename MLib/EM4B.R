...
def.Globals<-function()
{
  ...
  #provisorisch:
  modelDir<<- "EM4_FEB.1";
  bench="EXX50_RI";  xlsName="Data0E.xls";fundis=c(2:25); 
  experiment=define.experiments(bench=bench,path=modelDir)
...
def.Globals()
  ...
  if (F)
  {
    ...
    run.main(     bench="SUP500", xlsName="Data0S.xls",fundis=c(2:11), modelDir=modelDir    )
    
    run.main(     bench="DAX30",  xlsName="Data0D.xls", fundis=c(2:11),modelDir=modelDir    )
    run.main(     bench="NIKKEI_225", xlsName="Data0N.xls", fundis=c(2:11), modelDir=modelDir)
  }
  
  
modelDir<-"mm2016"  

  run.main(     bench="GOLD_AM_FIX_TROY_OZ",  xlsName="Data0Gold.xls", fundis=c(2:11),modelDir=modelDir)
  run.main(     bench="SILVER_FIX_TROY_OZ", xlsName="Data0Silv.xls", fundis=c(2:12), modelDir=modelDir)
  
  run.main(     bench="BMW_ST", xlsName="Data0BMW.xls", fundis=c(2:11), modelDir=modelDir)
  run.main(     bench="DAIMLER", xlsName="Data0DAIMLER.xls", fundis=c(2:11), modelDir=modelDir)
  run.main(     bench="VW", xlsName="Data0VW.xls", fundis=c(2:11), modelDir=modelDir)
  
  run.main(     bench="CARS_U_PARTS", xlsName="Data0CARS_PARTS.xls", fundis=c(2:11), modelDir=modelDir)
  run.main(     bench="EXX50_RI", xlsName="Data0E.xls",fundis=c(2:25),modelDir=modelDir)  
...
Auswertung<-function(bench="DAX30",xlsName="Data0D.xls")
{

  prepare.EM4B.data(file=sprintf("%s_em4b.data",bench), xlsName=xlsName,bench=bench,fundis=NULL,do.load=T,do.par=F, mkTrain=F,visual=F, modelDir=modelDir)
...
run.main<-function (bench="EXX50_RI",xlsName="Data0E.xls",fundis=c(2:25), modelDir="EM4_DEC.1")
{
  ...
  do.par=F
  prepare.EM4B.data(file=sprintf("%s_em4b.data",bench), xlsName=xlsName,bench=bench,fundis=fundis,do.load=do.load,do.par=do.par, mkTrain=T,visual=F, modelDir=modelDir)
  ...
  TrainRun_EM4B(Experi=experiment$macro.select.m , do.par=do.par)
  ...
  save(trainBest,file="trainBest.data")
  utils::write.table(trainBest,file="trainBest.csv",dec=".",sep=";",col.names=T,row.names=T)
...
define.experiments<-function(bench="EXX50_RI",path="EM4_DEC.1")
{
...
prepare.EM4B.data <-function(file="em4b.data",xlsName="Data0E.xls",bench="EXX50_RI",fundis=c(2:25), do.load=T,do.par=T,mkTrain=T,techbuilder.symbols=c(),visual=F,modelDir="EM4_DEC.1")
{
  if (do.load)
  {
  ...
  #PREP
  data <<-make_eckhard4B_data(visual=visual,xlsName=xlsName,bench=bench,do.par=do.par,mkTrain=F,fundis=fundis,modelDir=modelDir)
...
{ 
  ...
  cat(info,file=sprintf("%s/ExperimentInfo.txt",path),sep="\n")
  print(info)
...
auswertung<-function(Experi=experiment$macro.select, data=data, bestN=30,findMinimalFeatureSet=F)
{
  experi=  deparse(substitute(Experi))  
  experi=rightOf(experi,"\\$")
  mP("..............auswertung zu  %s",experi)
  ...
  no=lapply(1:len(symbolnames),function(sym.i) 
  {
    ...
    pdf(file = sprintf("%sZoomPlot_%s.pdf",path,sym), width = 8.5, height = 11)  
    layout(1:3)  
    colnames(model.err)
   # model.err.2=model.err[,-c("Index,signal,confidence,model.err,model.err.1,pred.err")]
    ...
    png(file = sprintf("%splotSigPrice_%s.png",path,sym), width = 1200, height = 600, units = 'px', pointsize = 12, bg = 'white')  
    plotSigPrice(signal=b[,1],prices=b[,2])#,indi=list(conf=merge(sig[,2])))
    ...
    utils::write.table(symSum,file=sprintf("%sSymbolWichtigkeit_%s.csv",path,sym),dec=".",sep=";",col.names=F,row.names=T)
    #.......................................................
    #k5=lapply(k4,function(c1)c1[c1 > 4]  )
    #k5=k4
    #die ListeNamen k5 als Dat?mer w?hlen
    #names(k5)<-index(df.q)
    #View(data.table(k5))
    ...
    if( bestN>0)
    {
      ...
      utils::write.table(train.best,file=sprintf("%strainTop%d%s.csv",path,bestN,sym),dec=".",sep=";",col.names=T,row.names=T)
      ...
      report.name=sprintf("%sdataset%s",path,sym)
      ...
      report.name=sprintf("%srfWichtigkeit%s.pdf",path,sym)
      ...
      cat(rpart.tree,file=sprintf("%s/rpart_tree.txt",path),sep="\n")
      ...
      utils::write.table(vimp,file=sprintf("%srpart_wichtigkeit_%s.csv",path,sym),dec=".",sep=";",col.names=F,row.names=T)
      plotcp(crs$rpart)
      ...
      report.name=sprintf("%srpart_Tree_%s.pdf",path,sym)
      pdf(file = paste(report.name,'.pdf',sep=""), width=8.5, height=11)
      ...
      if (F)
      {
        ...
        report.name=sprintf("%sj48Tree_%s.pdf",path,sym)
        ...
        cat(j48.tree,file=sprintf("%s/j48_tree.txt",path),sep="\n")
      }
      #................................................................
      library(party)
      ...
      report.name=sprintf("%scTree_%s.pdf",path,sym)
      ...
      cat(ctree,file=sprintf("%s/ctree.txt",path),sep="\n")
...
make_eckhard4B_data<-function(visual=F,bench="EXX50_RI",xlsName="Data0E.xls",do.par=T, mkTrain=F,fundis=c(2:25),modelDir="EM4_DEC.1")
{
  #einlesen von xls -sheets in xts- variable
  exx50.all= read.EckhardsXLS(modelDir=modelDir, xlsName=xlsName,startRow=5,belowColnames=5, date.format = "%d.%m.%Y",to.month.end=F,debug=F)  
  ...
  purePlot(fundamentale.faktoren,main="fundamentale")
  ...
  purePlot(intermarket.faktoren,main="intermarket")
...
small.cloud<-function(euro.macros,visual=F,cloud.control=list())
{
  ...
  #cloud.check(euro.macros.na)
  
  #lfaber.mac = rollRegressionXTS( na.omit(bt.apply.matrix(euro.macros.na,SMA,n=200)),win=201)
  #lfaber.mac = bt.apply.matrix(lfaber.mac, log)
  #lfaber.mac=col.rename(lfaber.mac,"lfaber")
  
  #if (visual)cloud.check(lfaber.mac)
  #if (visual)purePlot(lfaber.mac)
  ...
  rankFaber=col.rename(faber.mac,"rankFaber")
  #mPlots(rankFaber)
  ...
  #roc 30
  roc60M.mac=ROC(euro.macros.na+10000,60)
...
cloud.mac= (merge(  faber.mac,  rankFaber, slope200.mac,slope90.mac, roc60M.mac))
...
if (F)
{
  list_R_functions('MLib/EM4B.r')
  lapply(purl.examples('MLib/EM4B.r'), 
