...
{
  #3 lieferdaten-blöcke:   die TargetZeitreihe  bench (tagedaten), fundamentale faktoren (monats-daten mit lieferlag von 31 Tagen),  intermarket -tagedaten (devisenkurse, zinsen,...)
  do.par=F
  prepare.EM4B.data(file=sprintf("%s_em4b.data",bench), xlsName=xlsName,bench=bench,fundis=fundis,do.load=F,do.par=do.par, mkTrain=T,visual=F, modelDir=modelDir)
...
auswertung<-function(Experi=experiment$macro.select, data=data, bestN=30)
{
  experi=  deparse(substitute(Experi))  
  experi=rightOf(experi,"\\$")
  ...
  no=lapply(1:len(symbolnames),function(sym.i) 
  {
    ...
    if(bestN>0)
    {
      ...
      #Seite 5: finde zeitreihen die brutal hoch miteinander correliert sind und schlage einge zur löschung vor:
      ...
      class.pred <-  predict(crs$rf,type="class",newdata=na.omit(crs$dataset[crs$test, c(crs$input, crs$target)]))
      print("rainForest")
      class.pred=as.numeric(as.character(class.pred))
      print(  table(as.factor(Target[crs$test]),class.pred) )
      pred.err=1-sum(diag(table(as.factor(Target[crs$test]),class.pred)))/sum(class.pred)
...
make_eckhard4B_data<-function(visual=F,bench="EXX50_RI",xlsName="Data0E.xls",do.par=T, mkTrain=F,fundis=c(2:25),modelDir="EM4_DEC.1")
{
  ...
  fundamentale.faktoren= lag(fundamentale.faktoren,20)["1997::"] #hier schon mal gelagged weil die Teile ja einen Monat Lieferversp?tung haben
  #head(merge(fundamentale.faktoren[,1],fundamentale.faktoren.n[,1]),30)
  ...
  global_commission<<- 0.0015  #0.0005  #sys.Run() nimmt diese commission !!!!
...
small.cloud<-function(euro.macros,visual=F,cloud.control=list())
{
  ...
  #roc 30
  roc30M.mac=ROC(euro.macros.na+10000,30)
