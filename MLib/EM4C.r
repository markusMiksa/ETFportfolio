  ...
  #.... teste alternativen zu  random.forest
{
  ...
  load(file=sprintf("Models/%s/em4c.data",modelDir))
  
  
  for (xlsfile in all.xls.files)  {  
    #  xlsfile="600.xls"
    
    prepare.EM4C.data(file=sprintf("%s_em4c",xlsfile), xlsName=xlsfile,bench=bench,fundis=NULL,do.load=do.load,do.par=do.par, mkTrain=T,visual=F, modelDir=modelDir)#,techbuilder.symbols=c("Y"))
  }    
...
#speicher dann unter em4c.data
######################################################################################
make.em4c.data<-function()
{
  ...
  save(data,file=sprintf("Models/%s/em4c.data",modelDir))
...
prepare.all.xls<-function()
{
  ...
  for (xlsfile in all.xls.files)  {  #MMprepare
    ...
    prepare.EM4C.data(file=sprintf("%s_em4c",xlsfile), xlsName=xlsfile,bench=bench,fundis=NULL,do.load=F,do.par=do.par, mkTrain=T,visual=F, modelDir=modelDir)#,techbuilder.symbols=c("Y"))
  }
}
if (F)
  prepare.all.xls()
...
{ 
  ...
  if (F)
  {
    ...
    #xlsfile="600.xls"
    # prepare.EM4C.data(file=sprintf("%s_em4c.data",xlsfile), xlsName=xlsfile,bench=bench,fundis=NULL,do.load=T,do.par=do.par, mkTrain=T,visual=F, modelDir=modelDir)#,techbuilder.symbols=c("Y"))
    ##################################>> #MMTrainRun   
    def.Globals()
    for (xlsfile in all.xls.files)    #.............................
    {
...
read.many.xls.y<-function(modelDir=modelDir,universe="all.xls",bench="DAX30", safe="DAX30")
{
  ...
  for (xlsfile in all.xls.files)   #lassen sich die xls-files einlesen ?
  {
    mP("read.many.xls.y %s %s",modelDir,xlsfile)
...
if (F)
{
  mdata=read.many.xls.y(modelDir=modelDir,universe = "EM4C_Y",bench="STOXX_50",safe="STOXX_50")
}
#..................................................................
...
#.......................................................................................
def.Globals<-function(modeldir="EM4_MAERZ_C") #"EM4_MAERZ.2" "EM4_MAERZ_B"
{
...
Auswertung<-function(bench="DAX30",xlsName="600.xls")
{
  ...
  prepare.EM4C.data(file=sprintf("%s_em4c",xlsName), xlsName=xlsName,bench=bench,fundis=NULL,do.load=T,do.par=F, mkTrain=F,visual=F, modelDir=modelDir)
  
  #  ,            techbuilder.symbols=colnames(data$prices)       )
  #if (F)
  #   save(data,file="em4c.data")
...
run.main<-function (bench="EXX50_RI",xlsName="Data0E.xls",fundis=NULL, modelDir="EM4_DEC.1")
{
  ...
  if (schon.gemacht(file=sprintf("%s_em4c",xlsName),modelDir=modelDir,xlsName=xlsName,Experi=experiment$macro.select.m,mode="train"))
  {
  ...
  if (!schon.gemacht(file=sprintf("%s_em4c",xlsName),modelDir=modelDir,xlsName=xlsName,Experi=experiment,mode="prep"))
  {
    prepare.EM4C.data(file=sprintf("%s_em4c",xlsName), xlsName=xlsName,bench=bench,fundis=fundis,do.load=F,do.par=do.par, mkTrain=T,visual=F, modelDir=modelDir)
  }
  else
    prepare.EM4C.data(file=sprintf("%s_em4c",xlsName), xlsName=xlsName,bench=bench,fundis=fundis,do.load=T,do.par=do.par, mkTrain=T,visual=F, modelDir=modelDir)
  ...
  if (F)
  {
    TrainRun_EM4C(Experi=experiment$TEST , do.par=F)
    TrainRun_EM4C(Experi=experiment$TEST , do.par=T)
    TrainRun_EM4C(Experi=experiment$TEST , do.par=T)
    TrainRun_EM4C(Experi=experiment$macro.select , do.par=T)
  }
  TrainRun_EM4C(Experi=experiment$macro.select.m , do.par=do.par)
...
prepare.EM4C.data <-function(file="em4c",xlsName="Data0E.xls",bench="EXX50_RI",fundis=NULL, do.load=T,do.par=T,mkTrain=T,techbuilder.symbols=c(),visual=F,modelDir="EM4_DEC.1")
{
  file.res = sprintf("Models/%s/%s.data",modelDir,file)
  dir.create(dirname(file),recursive=T)
  mP("#### prepare.EM4C.data for %s #### ",file.res)
  
  if  (!do.load && file.exists(file.res))
  {
    mP("skip  prepare.EM4C.data:  result file  already exists \n %s ",file)
  ...
  if (do.load && !file.exists(file.res)) #evtl. wurde ja auf dem austauschlaufwerk alles bereit gelegt
  {
    ...
    mP("prepare.EM4C.data: load prepared file from \n %s ",file)
    
  }
  if (do.load && !file.exists(file.res)) #evtl. wurde ja auf dem austauschlaufwerk alles bereit gelegt
  {
    mP("skip  prepare.EM4C.data:  result prep-data not ready exists \n %s ",file)
    return("skip")
  } 
  ...
  if (do.load  )
  {
    ...
    if ( file.res=="Models/EM4_MAERZ.1/611.xls_em4c.data")
    {
      print("#MM1")
      #  browser()
    }
    local({
      ...
      if (is.null(data))
      {
        mP("prepare.EM4C.data: can't read file %s",file.res)
        return("bug")
      }
      print(ls(data))
      data<<-data
...
TSA.prepPro.e4C<-function(visual=T,bench="DAX30",do.par=T)
{
  ...
  TrainRun_EM4C(Experi=experiment$macro.select , do.par=F)
  TrainRun_EM4C(Experi=experiment$macro.select.m , do.par=F)
...
TrainRun_EM4C<-function(Experi=experiment$macro.select , do.par=do.par)
{ 
  #MMRUN
  ...
  {
    mP("skip TrainRun_EM4C, file exists: %s",infoFile)
    if (!global_ignore_schon_da) return("skip")
  }
  ...
  {
    #    sym=data$BENCH
    mP(">>TrainRun_EM4C %s %s",experi,sym)
...
make_eckhard4C_data<-function(visual=F,bench="EXX50_RI",xlsName="Data0E.xls",do.par=T, mkTrain=F,fundis=NULL,modelDir="EM4_DEC.1")
{
  #einlesen von xls -sheets in xts- variable
...
models = run_all_signal(SAFE=SAFE,do.assemble.Signals=F,file="test",LongShort="F",pdf=T,visual=F) #nur bei pdf=T werden auch die SA- und TSA- Modelle gerechnet
...
#... im rolling-mode
best=find.best.portfolio.model(models, period ="years",visual=T,rolling.mode=T,rank.for="mcalmar") #"",cagr, sharpe
...
#
print("########### load + source EM4C.R")
###############################################################################
if (F)
  list_R_functions('MLib/EM4C.R')
...
{
  list_R_functions('MLib/EM4C.r')
  lapply(purl.examples('MLib/EM4C.r'), 
