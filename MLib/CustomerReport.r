...
#require(rgarch)#macht nen bug sp?ter weil xts first,last ?berdeckt
#detach("package:xts")
#require(xts)
...
function (R, Rf = 0, main = NULL, submain=NULL, geometric=TRUE, methods = "none", width = 0, event.labels = NULL, ylog = FALSE, wealth.index = FALSE, gap = 12, begin=c("first","axis"), legend.loc="bottomleft", p=0.95, maxdraw = TRUE,...)
{ # @author Peter Carl
...
#    plot.new()
    layout(matrix(c(1,2,3)),height=c(3,1,1.3),width=1)
	...
	par(oma=c(2,1,2,2))	
    par(mar=c(1,4,2,4))
    chart.CumReturnsX(x, main = "", xaxis = FALSE, legend.loc = legend.loc, cex.legend = 0.8, event.labels = event.labels, ylog = ylog, wealth.index = wealth.index, begin = begin, geometric = geometric, ylab="Cumulative Return",...)
...
if (maxdraw) {
  abline(h=-0.2,col="tomato3",lwd=3,lty="dashed")
  text(x=2,y=-0.2,"maximum acceptable drawdown",adj = c(0, 0.5))
}
